...
## 2인 이상 멀티플레이어 단계별 실행 계획

상세 순서는 `docs/rearch/20260917-multiplayer-rollout-plan.md`에 기록한다. 초기 목표는 2개 기기에서 실제 한 판을 끝내는 최소 수직 슬라이스이며, 유료아이템은 후순위로 보류한다.

- [ ] M0. 공통 `GameState`/`GameAction`/`GameEvent`/`GameResult`와 서버 권위형 판정 경계 확정
- [ ] M1. 2인 방 생성·입장·ready·턴·행동·결과·재접속 최소 수직 슬라이스
- [ ] M2. 서로 다른 네트워크의 실제 기기 2대 E2E 검증
- [ ] M3. 3인 맞고, 쌍피·특수룰·GO/STOP·점수 서버 판정 확장
- [ ] M4. 4인 이상 관전자·광팔기·빠지기·탈주 처리
- [ ] M5. 윷놀이·뱀주사위·사다리·블랙잭·포커·슬롯 순서로 게임 모듈 확장
- [ ] M6. 실제 BGM/SFX·결과 공유·리플레이·운영 로그·비용 모니터링
- [ ] M7. 유료아이템은 장식형 가능성만 열어두고 별도 정책 검토 후 구현

초기 기본값: 무료 가상 게임머니, 현금/토스포인트 무관, 방 코드 초대 우선, Supabase 후보, 유료아이템 후순위.
